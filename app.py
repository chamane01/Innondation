...
def generate_depth_map(ax, grid_Z, grid_X, grid_Y, X_min, X_max, Y_min, Y_max, label_rotation_x, label_rotation_y):
    c = ax.pcolormesh(grid_X, grid_Y, grid_Z, shading='auto', cmap='viridis')
    ax.set_xlim(X_min, X_max)
    ax.set_ylim(Y_min, Y_max)
    ax.set_xlabel("X Axis")
    ax.set_ylabel("Y Axis")
    ax.tick_params(axis='x', rotation=label_rotation_x)
    ax.tick_params(axis='y', rotation=label_rotation_y)
    plt.colorbar(c, ax=ax, label="Depth")


# Fonction pour afficher les polygones
def afficher_polygones(ax, polygones):
...
def detecter_bas_fonds(grid_Z):
    return grid_Z < -10, None


# Simulation des données de la grille
grid_X, grid_Y = np.meshgrid(np.linspace(0, 100, 50), np.linspace(0, 100, 50))
grid_Z = np.random.uniform(-20, 10, grid_X.shape)  # Profondeurs aléatoires
...
if st.button("Afficher la carte et les objets"):
    # Charger les polygones et les routes
    polygones_dans_emprise = charger_polygones(uploaded_polygons_file) if uploaded_polygons_file else None
    routes_dans_emprise = charger_routes(uploaded_routes_file) if uploaded_routes_file else None
    ...
    generate_depth_map(ax, grid_Z, grid_X, grid_Y, X_min=0, X_max=100, Y_min=0, Y_max=100, label_rotation_x=0, label_rotation_y=-90)
