...
def main():
    ...
    if "fmap" not in st.session_state:
        ...
        st.session_state["fmap"].add_child(st.session_state["draw"])
        st.session_state["fmap"].add_child(MeasureControl())

    fmap = st.session_state["fmap"]

    # Téléversement du fichier MNT
    uploaded_mnt = st.file_uploader("Téléverser un fichier MNT (TIFF)", type=["tif", "tiff"])
    if uploaded_mnt:
        tiff_path = uploaded_mnt.name
