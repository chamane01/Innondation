...
# Charger le fichier DXF contenant les bâtiments
doc_batiments = ezdxf.readfile("batiments.dxf")  # Fichier DXF direct
...
# Sélection du site ou téléversement d'un fichier
...
if option_site == "AYAME 1":
    ...
    st.warning("Veuillez sélectionner un site ou téléverser un fichier.")
    df = None
...
if df is not None:
    ...
    if 'X' not in df.columns or 'Y' not in df.columns or 'Z' not in df.columns:
        ...
        batiments_inondes, batiments_non_inondes = 0, 0
        for entity in doc_batiments.modelspace().query('POLYLINE'):
            if entity.dxftype() == 'POLYLINE':
                points = [Point(x, y) for x, y, *_ in entity.get_points()]
                polygon = Polygon(points)
                if polygon.bounds[0] >= X_min and polygon.bounds[2] <= X_max and polygon.bounds[1] >= Y_min and polygon.bounds[3] <= Y_max:
                    centroid = polygon.centroid
                    interpolated_height = griddata((df['X'], df['Y']), df['Z'], (centroid.x, centroid.y), method=interpolation_method)
                    if interpolated_height <= st.session_state.flood_data['niveau_inondation']:
                        batiments_inondes += 1
                    else:
        ...
        if st.button("Afficher la carte d'inondation"):
            ...
            # Affichage des bâtiments inondés et non inondés
            for entity in doc_batiments.modelspace().query('POLYLINE'):
                points = [Point(x, y) for x, y, *_ in entity.get_points()]
                polygon = Polygon(points)
                if polygon.bounds[0] >= X_min and polygon.bounds[2] <= X_max and polygon.bounds[1] >= Y_min and polygon.bounds[3] <= Y_max:
                    centroid = polygon.centroid
                    interpolated_height = griddata((df['X'], df['Y']), df['Z'], (centroid.x, centroid.y), method=interpolation_method)
                    color = 'red' if interpolated_height <= st.session_state.flood_data['niveau_inondation'] else 'white'
            ...
            st.write(f"**Nombre de bâtiments en zone inondée :** {batiments_inondes}")
            st.write(f"**Nombre de bâtiments en zone non inondée :** {batiments_non_inondes}")
