...
# =============================
# MODULE SIG & RAPPORT
# (Fonctions issues du premier code)
# =============================

# --- Fonctions ReportLab et conversion d'images ---
...
# --- Fonctions d'analyse spatiale et de dessin ---
...
def create_map_sig(mosaic_file):
...
def run_analysis_spatiale_sig():
    st.title("🔍 Analyse Spatiale (SIG & Rapport)")
    st.info("Ce module vous permet de générer des contours à partir des zones dessinées ou de tracer des profils d'élévation.")
...
def run_report_sig():
    ...
    analysis_card = create_analysis_card_controller()  # fonction définie dans ce module (voir ci-dessous)
    if analysis_card:
    ...
    new_element = create_element_controller()  # fonction définie ci-dessous
    if new_element:
        elements.append(new_element)
        st.session_state["elements"] = elements
        st.success("Élément validé avec succès !")
    if elements:
        display_elements_preview(elements)  # fonction ci-dessous
...
# Fonctions utilitaires pour rapport SIG & Rapport
...
# --- Fin fonctions SIG & Rapport ---

# =============================
# MODULE CARTE TOPOGRAPHIQUE & ANALYSE SPATIALE
# (Fonctions issues de la deuxième partie)
# =============================

# Dictionnaire des couleurs pour les GeoJSON
geojson_colors = {
    "Routes": "orange",
    "Pistes": "brown",
    "Plantations": "green",
...
# Initialisations spécifiques pour ce module
...
def run_topographie():
    ...
    st.markdown("""
Créez des entités géographiques en les dessinant sur la carte et ajoutez-les à des couches spécifiques.  
...
def main():
    st.set_page_config(page_title="Application SIG & Topographie", layout="wide")
    mode = st.sidebar.radio("Choisissez le mode", options=["SIG & Rapport", "Carte Topographique & Analyse Spatiale"])
    if mode == "SIG & Rapport":
        sub_mode = st.sidebar.radio("Menu Principal", options=["Analyse Spatiale", "Rapport"], key="main_menu")
        if sub_mode == "Analyse Spatiale":
            run_analysis_spatiale_sig()
        elif sub_mode == "Rapport":
            run_report_sig()
    elif mode == "Carte Topographique & Analyse Spatiale":
        run_topographie()
