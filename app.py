...
if df is not None:
    ...
    if 'X' not in df.columns or 'Y' not in df.columns or 'Z' not in df.columns:
        ...
        if st.button("Afficher la carte d'inondation"):
            ...
            # Tracer le contour et la légende
            ...
            # Ajouter une légende
            ax.legend(['Contour d\'inondation', 'Zone inondée'], loc='upper right')

            # Ajouter des annotations pour les coordonnées
            for i in range(len(df)):
                ax.annotate(f"({df['X'][i]:.2f}, {df['Y'][i]:.2f})", xy=(df['X'][i], df['Y'][i]), fontsize=8, color='black')
