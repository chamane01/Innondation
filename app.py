...
from shapely.geometry import box  # Utilisé pour créer des polygones des bâtiments
...
# Options pour fichiers prédéfinis
...
if df is not None:
    ...
    if 'X' not in df.columns or 'Y' not in df.columns or 'Z' not in df.columns:
        ...
        if st.button("Afficher la carte d'inondation"):
            ...
            # Ajouter des bâtiments (polygones)
            batiments = [box(x, y, x+10, y+10) for x, y in zip(df['X'], df['Y'])]  # Exemple de polygones représentant des bâtiments
            batiments_inondes = 0

            for batiment in batiments:
                # Vérifier si le bâtiment intersecte la zone inondée
