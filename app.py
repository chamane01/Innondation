...
# Function to calculate flooded areas
def calculate_flooded_areas(dem_data, water_level):
    """Calculate flooded areas based on DEM data and water level."""
    flooded_areas = dem_data < water_level
...
def main():
    ...
    # Sidebar for flood parameters
    with st.sidebar:
        st.header("Paramètres d'inondation")
        water_level = st.slider("Niveau d'eau (m)", min_value=0.0, max_value=100.0, value=10.0, step=0.1)
        st.write("Définissez le niveau d'eau pour calculer les zones inondées.")
    ...
    # Bouton pour calculer et afficher les zones inondées
    if st.button("Calculer les zones inondées", key="calculate_flooded_areas"):
        # Trouver la couche MNT dans les couches téléversées
        ...
        if mnt_layer:
            with rasterio.open(mnt_layer["path"]) as src:
                dem_data = src.read(1)
                flooded_areas = calculate_flooded_areas(dem_data, water_level)
                
                # Convertir les zones inondées en une image overlay
                flooded_image = np.where(flooded_areas, 1, 0)
                flooded_image_path = "flooded_areas.png"
                plt.imsave(flooded_image_path, flooded_image, cmap="Blues", vmin=0, vmax=1)
                
                # Ajouter l'overlay à la carte
                add_image_overlay(fmap, flooded_image_path, mnt_layer["bounds"], "Zones inondées")
                st.success("Les zones inondées ont été calculées et affichées sur la carte.")
        else:
            st.error("Aucune couche MNT n'a été trouvée. Veuillez téléverser une couche MNT pour calculer les zones inondées.")
