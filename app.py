...
if df is not None:
    ...
    if 'X' not in df.columns or 'Y' not in df.columns or 'Z' not in df.columns:
        ...
        if st.button("Afficher la carte d'inondation"):
            ...
            # Charger les bâtiments OpenStreetMap dans la zone d'intérêt
            buildings = gpd.read_file("buildings.gpkg", bbox=(X_min, Y_min, X_max, Y_max)).to_crs("EPSG:32630")

            # Calcul des contours inondés comme union de polygones
            inondation_polygons = [Polygon(contour.get_paths()[0].vertices) for contour in contours_inondation.collections[0].get_paths()]
            zone_inondee = unary_union(inondation_polygons)

            # Classification des bâtiments et affichage
            buildings['inonde'] = buildings.geometry.apply(lambda x: x.intersects(zone_inondee))
            flooded_buildings = buildings[buildings['inonde']]
            non_flooded_buildings = buildings[~buildings['inonde']]

            flooded_buildings.plot(ax=ax, color="red", label="Bâtiments inondés")
            non_flooded_buildings.plot(ax=ax, color="white", edgecolor="black", label="Bâtiments non inondés")
            ...
            st.write(f"**Nombre de bâtiments inondés :** {len(flooded_buildings)}")
            st.write(f"**Nombre de bâtiments non inondés :** {len(non_flooded_buildings)}")
