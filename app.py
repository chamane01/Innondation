...
# Étape 1 : Fonction pour charger les fichiers GeoJSON
def upload_geojson():
    ...
    def _on_upload_change(change):
        if len(upload.value) > 0:
            ...
            return gpd.read_file(io.BytesIO(file_content))

    upload.observe(_on_upload_change, names='value')
    return upload

print("Téléversez le fichier des centroïdes (GeoJSON)")
centroid_gdf = None
upload_centroids = upload_geojson()

print("Téléversez le fichier des polygones (GeoJSON)")
polygon_gdf = None
upload_polygons = upload_geojson()

# Une fois les fichiers téléversés et chargés
centroid_gdf = gpd.read_file("centroids_file.geojson")  # Remplacez par les données téléversées
polygon_gdf = gpd.read_file("polygon_file.geojson")    # Remplacez par les données téléversées
...
# Étape 4 : Calcul des Points dans les Polygones
...
# Option avec tolérance (si nécessaire)
...
# Étape 5 : Affichage sur une carte
base = polygon_gdf.plot(color='white', edgecolor='black', figsize=(10, 8))
centroid_gdf.plot(ax=base, marker='o', color='red', markersize=5, label='Centroïdes')
points_in_polygon.plot(ax=base, marker='x', color='green', markersize=5, label='Centroïdes détectés')
plt.legend()
plt.title("Centroïdes et Polygones")
plt.show()
