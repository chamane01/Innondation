...
def create_map(mosaic_file):
    ...
    l'outil de dessin pour tracer des rectangles (pour les contours) et des lignes (pour les profils).
    """
...
def generate_contours(mosaic_file, drawing_geometry):
    """
    Génère des courbes de niveau à partir d'une zone (dessinée sous forme de rectangle).
...
def generate_profile(mosaic_file, coords, profile_title):
    """
    Génère un profil d'élévation le long d'une ligne dessinée.
...
def store_figure(fig, result_type, title):
    """
    Sauvegarde la figure matplotlib dans un buffer et ajoute le résultat
...
def run_analysis_spatiale():
    ...
    st.info("Générez vos cartes (contours ou profils) en dessinant sur la carte ci-dessous.\n\n**Important :** Une fois vos cartes générées, elles seront stockées pour le rapport. Veuillez ne pas quitter ce module avant d'avoir généré vos cartes.")
    
    # Initialisation du mode d'analyse (contours ou profils)
    if "analysis_mode" not in st.session_state:
        st.session_state["analysis_mode"] = "none"
    
    # On initialise les résultats d'analyse (les figures générées)
    if "analysis_results" not in st.session_state:
        st.session_state["analysis_results"] = []
    
    # Saisie du nom de la carte (affiché mais non utilisé pour la persistance)
    ...
    st.write("**Utilisez l'outil de dessin sur la carte ci-dessous pour générer vos analyses.**")
    map_data = st_folium(m, width=700, height=500, key="analysis_map")
    
    # Si l'utilisateur a dessiné, générer automatiquement la carte correspondante
    # Pour les rectangles, on génère les contours ; pour les lignes, on génère les profils.
    if map_data is not None and isinstance(map_data, dict) and "all_drawings" in map_data:
        drawings = map_data["all_drawings"]
        for drawing in drawings:
            geom = drawing.get("geometry", {})
            if geom.get("type") == "Polygon":
                # Génération des contours pour ce rectangle
                fig = generate_contours(mosaic_path, geom)
                if fig is not None:
                    st.pyplot(fig)
                    store_figure(fig, "contour", f"Contours - {datetime.now().strftime('%H:%M:%S')}")
            elif geom.get("type") == "LineString":
                # Génération d'un profil pour cette ligne
                profile_title = f"{map_name} - Profil {datetime.now().strftime('%H:%M:%S')}"
                fig = generate_profile(mosaic_path, geom.get("coordinates"), profile_title)
                if fig is not None:
                    st.pyplot(fig)
                    store_figure(fig, "profile", profile_title)
    
    # Options pour passer en mode "contours" ou "profils" (pour rafraîchir l'affichage si besoin)
    options_container = st.container()
    if st.session_state["analysis_mode"] == "none":
        col1, col2 = options_container.columns(2)
        if col1.button("Voir profils", key="btn_profiles"):
            st.session_state["analysis_mode"] = "profiles"
        if col2.button("Voir contours", key="btn_contours"):
            st.session_state["analysis_mode"] = "contours"
    
    # Vous pouvez ajouter ici d'autres contrôles pour l'analyse si nécessaire.
    st.markdown("**Les cartes générées sont automatiquement sauvegardées pour le rapport dans la section 'analysis_results'.**")
...
def create_element_controller():
    # Permet d'ajouter un élément personnalisé (texte ou image importée)
...
def run_report():
    ...
    # Utilisation des cartes générées (analysis_results) pour le rapport
    ...
    if "analysis_results" in st.session_state and st.session_state["analysis_results"]:
        ...
        # Filtrage des doublons pour éviter les répétitions
        unique_results = []
        unique_keys = set()
